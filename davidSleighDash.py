...
# Big coal
BIG_COAL_SIZE = 100
BIG_COAL_SPEED_FACTOR = 0.5  # moves slower than normal coal

# Snowflake
SNOWFLAKE_DRIFT = 1

# Updated durations:
BASE_SPEED_CHANGE = 2         # Speed up/down adjustment
POWERUP_DURATION = 10000      # Speed up/down effect now lasts 10 seconds
POWERUP_CHANCE = 0.15         # Probability of spawning a normal power-up each frame
ITEM_FALL_SPEED = 3           # How fast items (power-ups) fall

SLEIGH_LIVES = 3              # Starting lives

SHIELD_DURATION = 5000        # Shield active time (ms) remains 5 seconds
FREEZE_DURATION = 7000        # Freeze active time updated to 7 seconds
MAGNET_DURATION = 7000        # Magnet active time updated to 7 seconds

# Timers for introducing Big Coal if no life lost
TIME_FOR_BIG_COAL = 20        # Survive 20s -> big coal warning
WARNING_DURATION = 3000       # 3s warning message

# Updated special events durations:
BLIZZARD_DURATION = 15000     # Blizzard lasts 15 seconds
METEOR_SHOWER_DURATION = 15000  # Meteor shower lasts 15 seconds
METEOR_SPEED_BOOST = 3

# Colors
WHITE  = (255, 255, 255)
RED    = (255,   0,   0)   # Speed Up
BLUE   = (  0,   0, 255)   # Speed Down
GOLD   = (255, 215,   0)   # Shield
CYAN   = (  0, 255, 255)   # Freeze
GRAY   = (128, 128, 128)   # Magnet
PINK   = (255,  20, 147)   # Health Restore
BLACK  = (  0,   0,   0)
GREEN  = (  0, 255,   0)   # Special Event

# -------------- Set up the screen & images -------------- #
...
# ----------------- Helper: Smaller Hitbox ----------------- #
def get_sleigh_hitbox(sleigh_rect):
    """
    Return a smaller rectangle inside the full sleigh_rect to avoid collisions.
    """
    hitbox_padding_x = 20
    hitbox_padding_y = 30
    return pygame.Rect(
...
def create_coal_set(num_coal):
    """Start the game with normal coal only."""
    return [create_normal_coal() for _ in range(num_coal)]

# ----------------- Items (Power-Ups & Special Event Trigger) ----------------- #
def create_item():
    """
    Randomly create a power-up item (red, blue, gold, cyan, gray, pink, or green).
    """
...
def apply_blizzard_overlay(surface):
    ...
    overlay.fill((0, 0, 0, 100))  # alpha ~100 dims the screen
    surface.blit(overlay, (0, 0))

# ----------------- Collision Checks ----------------- #
def colliding_obstacle(sleigh_rect, obstacles):
    """
    Return the *first* obstacle that collides with the sleigh,
...
def check_collision_sleigh_items(sleigh_rect, items):
    """
    Check for collision between the sleigh and any items.
    Returns the index of the colliding item or -1 if no collision.
    """
    sleigh_hitbox = get_sleigh_hitbox(sleigh_rect)
...
# ----------------- Main Game Logic ----------------- #
def run_game():
    # For freeze logic: store speeds in a dictionary to avoid mismatch
    original_speeds_dict = {}

    life_lost_message = ""
    life_lost_message_timer = 0
    message_display_duration = 2000  # Message duration (ms)
    ...
    # We'll track snowflakes created by blizzard so we can remove them 
    ...
    while running:
        ...
        # Check collisions with items => apply effect
        ...
        # If event warning is done, pick an event
        ...
        # If an event is active, check if it ended
        ...
        if freeze_active and current_time > freeze_end_time:
            freeze_active = False
            # Revert each obstacle by matching dictionary IDs
            for o in obstacles:
                obs_id = id(o)
...
def show_game_over(score):
    """
    Displays the game over screen with the 'davidEndScreen.png' figure pointing to the score.
    """
